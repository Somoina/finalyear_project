function [reg_properties, reg_properties_mean] = Chain_f_res(I,I_fluo,pic)
% count = 0;
...
I_fl = mat2gray(I_fluo);
I2_ = adapthisteq(I_contour,'NumTiles',[40 40],'clipLimit',0.01,'Distribution','rayleigh','Alpha',0.8);

% for the bounding box
I_contour_ = mat2gray(I_bb);
I2 = adapthisteq(I_contour_,'NumTiles',[40 40],'clipLimit',0.01,'Distribution','rayleigh','Alpha',0.8);
% figure;imhist(I2_);
...
I3_ = imdilate(I2_, offsetstrel('ball',7,0));figure;imshow(I3_);
I3 = imdilate(I_contour_, offsetstrel('ball',70,0));figure;imshow(I3);
% imshow(I3_);
% figure;
...
thresh_ = thresh_*1.06;    
%to find the bounding box
thresh_bb = graythresh(I3);

I4_ = (imbinarize(I3_,thresh_));
I4 = (imbinarize(I3,thresh_bb));
...
for i=1:n
      ...
      idx_x=[bb_i(1)-4 bb_i(1)+bb_i(3)+4];
      idx_y=[bb_i(2)-4 bb_i(2)+bb_i(4)+4];
      ...
      ImCell{i} = I_contour(idx_y(1):idx_y(2),idx_x(1):idx_x(2));
      FlCell{i} = I_fl(idx_y(1):idx_y(2),idx_x(1):idx_x(2));
end
...
%mean intensity for fl
mean_intensity_0_fl = regionprops(I9,I11, 'MeanIntensity');
mean_intensity_fl = mean([mean_intensity_0_fl.MeanIntensity]);
...
%fluorescence of region
features_4 = regionprops(label_0,I11, 'MeanIntensity');
...
%get the fluorescence of the border region
features_3 = regionprops(label_1,I11, 'MeanIntensity');
feature_fl = zeros(m,1);
...
for i = 1:m
    ...
    feature_fl(i,1) = features_3(i).MeanIntensity/mean_intensity_fl;
    %find regions bordering the boundary
    [juu, chini, kushoto, kulia] = find_cell_frag(i, label_0, label_1);
    spread(i,:) = region_stats(juu, chini, kushoto, kulia, features, features_0, features_4, feature_width_1(i,1), mean_intensity,mean_intensity_fl);
    no_info(i,:) = [pic, i];
...
the_features_0 =  horzcat(spread, features_intensity_1,feature_width_1,feature_fl,no_info);
the_features_1 =  horzcat(spread,features_intensity_1,feature_width_1,feature_fl);
figure;
plot_label(label_1, IM_border, RGB_label_0);
%xlswrite('6_fea.xlsx',the_features_0);
close all
%% Visual Outputs
v = evalin('base', 'trainedClassifier5');
...
%% Get the properties of the r  egions
[reg_property, reg_property_mean, pic_info] = getResults(label_2,I11, pic);
reg_properties = horzcat(pic_info,reg_property);
reg_properties_mean = horzcat(reg_property_mean);
end