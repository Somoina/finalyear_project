...
%the variable I, ensure that the image folder is added to Matlab path

% I = imread('sal15_9-p-074.tif');
I = imread('62_44.tif');
pic = 25;
...
%close all
I_contour = I;
I2_ = adapthisteq(I_contour,'NumTiles',[100 100],'clipLimit',0.045,'Distribution','rayleigh','Alpha',0.7);
figure;imhist(I2_);
...
I3_ = imclose(I2_, strel('ball',35,60));
%imshow(I3_);
figure;
...
%% Visual Outputs
v = evalin('base', 'Class.trainedClassifier_1');
...
IM_NN = bwareaopen(IM_NN, 10);
%imshow(IM_NN);
%figure;
B = bwboundaries(IM_NN);
cc_2 = bwconncomp(IM_NN, 8);
